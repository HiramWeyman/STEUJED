...
export class ArchivosFormComponent implements OnInit {
  ...
  onUpload(){
    ...
    this.http.post(`${this.urlEndPoint+"/UploadFiles?ruta="+this.selectedFile.name}`,fd,{
      reportProgress: true,
      observe: 'events'
    })
      .subscribe(event => {
        if (event.type === HttpEventType.UploadProgress){
          ...
          if (event.status == 201){
            //////registro
            this._archivos.create(this.archivos,this.selectedFile.name,'1').subscribe(archivo =>{
              this.router.navigate(['/archivos']);
                Swal.fire('Guardado', `Archivo ${archivo.archivo_descrip} creado con éxito!`, 'success');
            },
            error => {
